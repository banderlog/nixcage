...
### 2.1 Host prerequisites
 ...
 | Dependency                                 | Purpose                                          |
 | -----------------                          | ------------------------------------------------ |
 | Nix (with flakes)                          | Build the VM image                               |
 | ssh, ssh-keygen                            | VM control plane                                 |
 | python3 or perl                            | Free-port selection at `init` time               |
 | KVM                                        | cloud-hypervisor acceleration                    |
 | bash, coreutils, grep, sed, awk, sha256sum | Script runtime                                   |

The tool itself is a single Bash script.
...
| `HYPERVISOR`  | `cloud-hypervisor` (Linux)                                         |
| `SHARE_PROTO` | `virtiofs` (Linux)                                                 |
...
- `bash`, `openssh` in `environment.systemPackages`.
...
- **Trust in microvm.nix and the hypervisor.** A guest escape in cloud-hypervisor defeats the boundary.
